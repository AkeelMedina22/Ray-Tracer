...
RGBColor Matte::shade(const ShadeInfo& si) const {
    Vector3D wo = -si.ray.d;   // from obj to camera
    RGBColor L = ambient_brdf->rho(si, wo); // (kd * cd)
    int num_lights = si.w->lights.size();

    // check all the lights
    for (int i = 0; i < num_lights; i++) {
        Vector3D wi = si.w->lights[i]->get_direction(si);
        float ndotwi = si.normal * wi; // dot prod of reflected light dir and normal at hitpoint

        if (ndotwi > 0.0) {
            bool in_shadow = false;

            // if want to create shadows - castshadows return true or false
            if (si.w->lights[i]->casts_shadows()) {
                Ray shadowRay(si.hit_point, wi);
                in_shadow = si.w->lights[i]->in_shadow(shadowRay, si); // shadow ban raha hai y anahi i think
            }
            if (!in_shadow)
                 L += diffuse_brdf->f(si, wo, wi) * si.w->lights[i]->L() * ndotwi;
                 // (kd * cd * invPi) * (ls * color) * (float) 
        }
    }
    return (L);
}
